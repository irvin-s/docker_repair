...
COPY . /build
RUN dotnet publish src/DaaSDemo.Api/DaaSDemo.Api.csproj /p:VersionPrefix=${VERSION_PREFIX} /p:VersionSuffix=${VERSION_SUFFIX} -o /out
...
ENTRYPOINT [ "dotnet", "DaaSDemo.Api.dll" ]
