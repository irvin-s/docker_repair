...
ADD https://api.github.com/repos/nekonium/cpp-nekonium/git/refs/heads/develop unused.txt 
...
RUN cd cpp-nekonium/build && cmake .. -DCMAKE_BUILD_TYPE=Debug -DVMTRACE=1 -DPARANOIA=1 -DEVMJIT=1 && make -j $(cat /proc/cpuinfo | grep processor | wc -l) && make install
RUN ldconfig

ENTRYPOINT ["/cpp-nekonium/build/test/checkRandomStateTest"]
