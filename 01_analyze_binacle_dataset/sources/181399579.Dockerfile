...
ENV DOCKER_VERSION=1.7.1 GPG_KEY_ID=A87A2270 TUTUM_REPO=https://repo.tutum.co/ubuntu/ CERT_COMMON_NAME=*.node.tutum.io DOCKER_HOST=unix:///var/run/docker.sock DOCKER_HOST_LISTEN=tcp://0.0.0.0:2375 TUTUM_HOST=https://dashboard.tutum.co/
...
# Workaround for 1.7.1 failing if cacert is not found
RUN mkdir -p /root/.docker && touch /root/.docker/ca.pem