# Adjusted from https://github.com/nekonium/go-nekonium/blob/develop/Dockerfile
FROM ubuntu:14.04
...
## Fetch and install go-nekonium
RUN go get -u -v -d github.com/nekonium/go-nekonium/...
WORKDIR $GOPATH/src/github.com/nekonium/go-nekonium

RUN git checkout develop
RUN git pull


RUN ETH_DEPS=$(go list -f '{{.Imports}} {{.TestImports}} {{.XTestImports}}' github.com/nekonium/go-nekonium/... | sed -e 's/\[//g' | sed -e 's/\]//g' | sed -e 's/C //g'); if [ "$ETH_DEPS" ]; then go get $ETH_DEPS; fi
