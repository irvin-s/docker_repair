...
EXPOSE 8080
ADD target/shovel-0.1.3-standalone.jar /srv/shovel-0.1.3-standalone.jar
ADD conf/app.edn /srv/conf/app.edn
RUN cat /srv/conf/app.edn
CMD ["java", "-jar", "/srv/shovel-0.1.3-standalone.jar", "print-config", "--config-file", "/srv/conf/app.edn"]
