...
#
# docker build --build-arg uid=$UID -t t-rex-trusty -f Dockerfile-trusty .
# docker run -t -i -v $(pwd):/var/data/in t-rex-trusty
# 
# Copy generated DEB package
# docker run --entrypoint="" t-rex-trusty ls -l /
# docker run --entrypoint="" -v $(pwd):/var/data/out t-rex-trusty cp /t-rex-0.8.0-dev-amd64-ubuntugis~trusty.deb /var/data/out/
...
>debian/DEBIAN/control && \
    ...
    mv debian.deb t-rex-$VERSION-$ARCH-ubuntugis~trusty.deb && \
    echo t-rex-$VERSION-$ARCH-ubuntugis-trusty.deb

RUN dpkg -i t-rex-*-ubuntugis~trusty.deb && rm -rf debian
