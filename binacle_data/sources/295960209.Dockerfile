...
# Hadoop
RUN wget -q -o out.log -P /tmp https://archive.apache.org/dist/hadoop/common/hadoop-2.6.5/hadoop-2.6.5-src.tar.gz && \
  tar xzf /tmp/hadoop-2.6.5-src.tar.gz -C /opt && \
  rm /tmp/hadoop-2.6.5-src.tar.gz && \
  mv /opt/hadoop-2.6.5-src /opt/hadoop
...
RUN . ~/.bashrc && \
  cd /opt/spark && \
  mvn clean -Dhadoop.version=2.6.5 -Pyarn -Phadoop-2.6 -DskipTests clean package
