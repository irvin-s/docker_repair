...
# docker run --volume=something.ovpn:/ovpn.ovpn:ro --device=/dev/net/tun --cap-add=NET_ADMIN
...
ENTRYPOINT [ \
    "/bin/bash", "-c", \
    "cd /etc/openvpn && /usr/sbin/openvpn --config *.ovpn --script-security 2 --up /usr/local/bin/sockd.sh" \
    ]
