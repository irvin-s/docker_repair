...
## Deploy php 5.3 role with ansible, we using ansible in prod - not yet docker
RUN cd /tmp/ && \
    git clone https://github.com/vukor/ansible-deploy-web-stack && \
    cd /tmp/ansible-deploy-web-stack/ && \
    echo 'local' >> inventory/hosts && \
    sed -i 's/php_version: 5.6/php_version: 5.3/' inventory/group_vars/all.yml && \
    sed -i 's/prj_group: www/prj_group: dev/' inventory/group_vars/all.yml && \
    ansible-playbook playbook/setup.yml -t unix,user,php --connection=local && \
