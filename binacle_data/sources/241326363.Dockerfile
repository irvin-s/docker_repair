...
#
# docker run --rm supinf/container-struct-test:1.1 test -h
# docker run --rm --privileged -v /var/run/docker.sock:/var/run/docker.sock \
#     -v $(pwd):/config supinf/container-struct-test:1.1 \
#     -image supinf/awscli:1.15 /config/test.yaml

FROM docker:18.03.0-ce-dind

ENV CONTAINER_STRUCTURE_TEST_VERESION=1.1.0
