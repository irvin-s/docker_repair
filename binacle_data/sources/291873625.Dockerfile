...
# Image Metadata
LABEL com.blacklabelops.application.cronium-cloud.docker.version=$DOCKER_VERSION \
      com.blacklabelops.application.cronium-cloud.docker-machine.version=$DOCKER_MACHINE_VERSION \
      com.blacklabelops.image.builddate.cronium-cloud=${BUILD_DATE}

COPY imagescripts /opt/cloud
ENTRYPOINT ["/sbin/tini","--","/opt/cloud/docker-entrypoint.sh"]
CMD ["cronium"]
