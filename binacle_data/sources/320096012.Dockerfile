...
# kcptun config
ENV VERSION 20170930
ENV MODE server
ENV listen_port 4000
ENV remote_server 127.0.0.1
ENV remote_port 8989

RUN apk --no-cache add git python libsodium wget tar nginx bash && \
    ...
    mkdir /run/nginx && mkdir /var/www/digitalworld && \
    apk del --purge git wget tar

# setup website
COPY index.html /var/www/digitalworld/index.html
