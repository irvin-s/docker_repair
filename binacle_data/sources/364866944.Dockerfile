# This container builds the rumprun toolchain for xen/i686
...
RUN DESTDIR=/usr/local && \
    BUILDRUMP_EXTRA="-F ACLFLAGS=-m32 -F ACFLAGS=-march=i686" && \
    git clone https://github.com/rumpkernel/rumprun && \
    cd /build/rumprun && \
    git submodule init && git submodule update && \
    ./build-rr.sh -d $DESTDIR -o ./obj xen build -- $BUILDRUMP_EXTRA && \
    sudo ./build-rr.sh -d $DESTDIR -o ./obj xen install && \
    rm -rf /build/rumprun
