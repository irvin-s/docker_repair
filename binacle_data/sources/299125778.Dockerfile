# adjusted from https://github.com/dbix-project/cpp-dubaicoin/blob/develop/docker/Dockerfile
FROM ubuntu:14.04
...
ADD https://api.github.com/repos/dubaicoin/cpp-dubaicoin/git/refs/heads/develop unused.txt
...
RUN cd cpp-dubaicoin/build && cmake .. -DCMAKE_BUILD_TYPE=Debug -DVMTRACE=1 -DPARANOIA=1 -DEVMJIT=1 && make -j $(cat /proc/cpuinfo | grep processor | wc -l) && make install
RUN ldconfig

ENTRYPOINT ["/cpp-dubaicoin/build/test/checkRandomStateTest"]
