...
VOLUME /tmp
LABEL Vendor="EasyFrame" easyframe-serv="ef-dataflowserver"
ADD target/easyframe-dataflow-server-master-SNAPSHOT-exec.jar dataflowserver.jar
RUN sh -c 'touch /dataflowserver.jar'
...
ENTRYPOINT [ "sh", "-c", "java $JAVA_OPTS -Djava.security.egd=file:/dev/./urandom -jar /dataflowserver.jar" ]